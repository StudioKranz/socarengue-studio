# Claude Code Instructions

Read `AGENTS.md` first. It is the canonical project instruction file.

Then read:

- README.md
- docs/design-language.md
- docs/content-model.md
- docs/studio-implementation-brief.md
- docs/artifact-intake-workflow.md, if present

Do not treat demo seed data as canon.
Do not commit private or unreleased story material.
Do not apply a global root redirect.
Keep socarengue.com public and socarengue.studio internal.
Run `pnpm typecheck` and `pnpm build` before proposing a commit.